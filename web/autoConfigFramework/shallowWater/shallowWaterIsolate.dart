...
main(List<String> args, SendPort sendPort) {
  ...
  requestEdgeValues(){
    //print("$x , $y ");
    if(_xp1 != null){
      _xp1.send(["requestEdge","_xp1"]);
    }
    if(_xm1 != null){
      _xm1.send(["requestEdge","_xm1"]);
    }
    if(_yp1 != null){
      _yp1.send(["requestEdge","_yp1"]);
    }
    if(_ym1 != null){
      _ym1.send(["requestEdge","_ym1"]);
    }
  }

  updateEdges(Map<String, SendPort> edgeMap){

    if(edgeMap.containsKey("_xp1")){
      _xp1 = edgeMap["_xp1"];
    }
    if(edgeMap.containsKey("_xm1")){
      _xm1 = edgeMap["_xm1"];
    }
    if(edgeMap.containsKey("_yp1")){
      _yp1 = edgeMap["_yp1"];
    }
    if(edgeMap.containsKey("_ym1")){
      _ym1 = edgeMap["_ym1"];
    }
    requestEdgeValues();
  }

  sendEdgeValue(String edge){


    //used for debugging linked edges across different isolates
    //print(x);
    int size = tileRes;
    ...
    switch(edge){
      case "_xp1" :
        if(_xm1 != null) {
          int selfPos = (size * size) - (size * 2);

          List edgeList = new List();
          edgeList = h.sublist(selfPos, selfPos + size);

          _xm1.send(["newEdge","_xp1", edgeList]);
        }
        break;
      case "_xm1" :
        if(_xp1 != null) {
          ...
          _xp1.send(["newEdge","_xm1", edgeList]);
        }
        break;
      case "_yp1" :
        if(_ym1 != null) {
          //_ym1.send(["newEdge","_yp1", [7,8,9]]);
        }
        break;
      case "_ym1" :
        if(_yp1 != null) {
          //_yp1.send(["newEdge","_ym1", [10,11,12]]);
        }
        break;
      default :
        break;
  ...
  updateEdgeValues(String edge, List newEdgeData){
  ...
  shallowWaterSetup(List<List<int>> baseLayout) {
    g = new List<double>(tileRes * tileRes);
    h = new List<double>(tileRes * tileRes);
    h1 = new List<double>(tileRes * tileRes);
    u = new List<double>(tileRes * tileRes);
    u1 = new List<double>(tileRes * tileRes);
    v = new List<double>(tileRes * tileRes);
    v1 = new List<double>(tileRes * tileRes);

    b = new List<bool>(tileRes * tileRes);
  ...
  shallowWaterCreation(List grid) {
    List _positions = new List();
    _elements = new List<int>();
    List _normals = new List();

    //Create the vertices only if
    for (double i = 1.0; i < grid.length - 1; i++) {
      for (double j = 1.0; j < grid[i.toInt()].length - 1; j++) {
        _positions.add(i);
        _positions.add(10.0);
        _positions.add(j);
      }
    ...
    for (double i = 1.0; i < grid.length - 1; i++) {
      for (double j = 1.0; j < grid[i.toInt()].length - 1; j++) {
        _positions.add(((i - 1.0 + (x * tileRes)) - x) / 4);
        _positions.add(0.5);
        _positions.add(((j - 1.0 + (y * tileRes)) - y) / 4);
      }
    }

    _normals = createNormals(_elements, _positions);

    sendPort.send(["init", _positions, _normals, _elements]);
  }

  shallowWaterUpdate(double dt) {
    ...
    for (int i = 1; i < X+1; i++) {
      for (int j = 1; j < Y+1; j++) {
        _positions.add(((i - 1.0 + (x * tileRes)) - x) / 4);
        _positions.add(h[(i-1) * X + (j-1)] * 10.0);
        _positions.add(((j - 1.0 + (y * tileRes)) - y) / 4);
    ...
    _normals = createNormals(_elements, _positions);
    sendPort.send([_posToSend, _normals]);
  ...
  receivePort.listen((msg) {
    //sendPort.send("u");
    //sendPort.send(["init", tileResolution, this.baseLayout]);
    //print(msg);
    if (msg[0] == "init") {
      tileRes = msg[1];
      x = msg[3];
      y = msg[4];
      shallowWaterSetup(msg[2]);
      shallowWaterCreation(msg[2]);
    } else if (msg[0] == "update") {
      //print("update");
      shallowWaterUpdate(msg[1]);
    } else if (msg[0] == "edges") {
      updateEdges(msg[1]);
    } else if(msg[0] == "requestEdge") {
      sendEdgeValue(msg[1]);
    } else if (msg[0] == "newEdge"){
      updateEdgeValues(msg[1], msg[2]);
    }else {
      print(msg);
    }
  });
